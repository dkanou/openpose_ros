...
x_RRHN, x_RRHF, x_RRHT = 0.437191 - torso_2_x, 0.973348 - torso_2_x, 0.630994 - torso_2_x
y_RRHN, y_RRHF, y_RRHT = -0.369656 - torso_2_y, -0.238957 - torso_2_y, -0.830075 - torso_2_y
z_RRHN, z_RRHF, z_RRHT = 0.638777 - torso_2_z, 1.38092 - torso_2_z, 1.34539 - torso_2_z
sx, sy, sz = 1, 1, 1
...
def calibration_callback(msg):
    global counter, T_calib_bool, N_calib_bool, F_calib_bool, H_calib_bool, robot_out, sx, sy, sz, depth_phase_done, depth
    name = 'right_wrist'
    step = 0.6
    m = getattr(msg, name)
    ...
    if(msg.num_people_detected == 1):    
        ...
        if counter < n_counter:
            ...
            if NanBool:
                ...
                diff_coords['right_wrist'] = (human_coords['right_wrist'][0] - m.x,
                                              human_coords['right_wrist'][1] - m.y,
                                              human_coords['right_wrist'][2] - m.z)
                human_coords['right_wrist'] = (m.x, m.y, m.z)
                dx = diff_coords['right_wrist'][0] * sx#.get(depth) #sx([depth])[0]
                dy = diff_coords['right_wrist'][1] * sy#.get(depth) #sy([depth])[0]
                dz = diff_coords['right_wrist'][2] * sz#.get(depth) #sz([depth])[0]
                scaled_coords['right_wrist'] = (-dz , -dx, -dy)
                robot_prev['right_wrist'] = (robot_coords['right_wrist'][0] + scaled_coords['right_wrist'][0],
                                             robot_coords['right_wrist'][1] + scaled_coords['right_wrist'][1],
                                             robot_coords['right_wrist'][2] + scaled_coords['right_wrist'][2])
